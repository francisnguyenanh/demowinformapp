...
function Get-ColumnValue {
    ...
    $cellFix = $ColumnInfo.CELL_FIX
    $cellLogic = $ColumnInfo.CELL_LOGIC
    ...
    switch ($columnName) {
        ...
        "SEQ" { return "$SeqValue" }
        "SEQ_K" { return "$SubSeqValue" }
        "SEQ_F" { return "$SubSeqValue" }
        "SEQ_RE" { return "$SubSeqValue" }
        "SEQ_CSV" { return "$SubSeqValue" }
        "SHEET_NAME" { return "N'$SheetName'" }
        "PROCESSOR_TYPE" { return "'koumoku'" }
        "LOGIC_TYPE" { return "'koumoku_logic'" }
    }
    ...
    if ($cellFix -and $cellFix -ne "") {
        try {
            $cellValue = Get-CellValue -SheetName $SheetName -CellAddress "$cellFix"
            if ($cellValue -and $cellValue -ne "") {
                if ($dataType -eq "nvarchar") {
                    return "N'$($cellValue.Replace("'", "''"))'"
                } else {
                    return "'$cellValue'"
                }
            }
    ...
    if ($cellLogic -and $cellLogic -ne "") {
        try {
            $cellValue = Get-CellValue -SheetName $SheetName -CellAddress "${cellLogic}$RowNum"
            if ($cellValue -and $cellValue -ne "") {
                if ($dataType -eq "nvarchar") {
                    return "N'$($cellValue.Replace("'", "''"))'"
                } else {
...
function Process-MenuData {
    ...
    $inserts = @()
    $tableName = $script:RowProcessorConfig['menu']['table_name']
    
    if ($script:TableInfo.$tableName) {
        $insertStatement = New-InsertStatement -TableName $tableName -ColumnsInfo $script:TableInfo.$tableName -SheetName $SheetName -RowNum 10 -SeqValue $SeqValue -ProcessorType "menu"
        $inserts += $insertStatement
    }
